...
def analyze_pipeline_timing(frame, iterations=100):
    ...
    for i in range(iterations):
        ...
        # Step 6a: Recognize left digit
        t0 = time.perf_counter()
        left_digit, left_score, _ = recognize_digit_template(left_digit_img, auto_learn=False, return_debug=True)
        t1 = time.perf_counter()
        timings['recognize_left'].append((t1 - t0) * 1000)

        # Step 6b: Recognize right digit
        t0 = time.perf_counter()
        right_digit, right_score, _ = recognize_digit_template(right_digit_img, auto_learn=False, return_debug=True)
        t1 = time.perf_counter()
...
def print_timing_report(stats):
    ...
    if 'total' in stats:
        s = stats['total']
        print(f"{'TOTAL':<20} {s['mean']:>10.3f}   {s['std']:>10.3f}   {s['min']:>10.3f}   {s['max']:>10.3f}")
    ...
    print("\n" + "=" * 60)
    fps = 1000 / stats['total']['mean'] if stats.get('total', {}).get('mean', 0) > 0 else 0
